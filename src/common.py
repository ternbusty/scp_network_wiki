...
jp_url = 'https://scp-jp.wikidot.com/'
...
def correct_unknown_rate(key: str, obj: dict, base_url: str = en_url) -> None:
    ...
    for original_link in original_links:
        ...
        return
    res = requests.get(base_url + redirect_link)
...
def create_obj_from_link(
        link: str,
        excludes: list = [],
        base_url: str = en_url) -> dict:
...
def iterate_and_save_link_list(
    ...
    for i in range(start, len(link_list)):
        link = link_list[i]
        tmp_obj = create_obj_from_link(
            link, excludes=[link], base_url=base_url)
        if tmp_obj is None:
            continue
        print(tmp_obj)
        print(i, '/', len(link_list), tmp_obj[link]['title'])
        if tmp_obj[link]['rate'] == 'unknown':
            correct_unknown_rate(link, tmp_obj, base_url=base_url)
...
def get_and_save_metatitle(
    ...
    with open(filename + '.yaml', mode) as file:
        yaml.dump(obj, file)
    sleep(random() * 10 + 2)
