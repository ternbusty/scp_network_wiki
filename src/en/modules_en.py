...
class SCP():
    ...
    def load_yaml(self):
        with open(self.path + 'scp_en.yaml') as file:
            self.scp_en_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'scp_001_en.yaml') as file:
            self.scp_001_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'ex_scp_en.yaml') as file:
            self.ex_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'arc_scp_en.yaml') as file:
            self.arc_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'd_scp_en.yaml') as file:
            self.d_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'j_scp_en.yaml') as file:
            self.j_obj = remove_adult(yaml.safe_load(file))

        with open(self.path + 'metatitle_en.yaml') as file:
            self.scp_en_mt_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'ex_metatitle_en.yaml') as file:
            self.ex_mt_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'arc_metatitle_en.yaml') as file:
            self.arc_mt_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'd_metatitle_en.yaml') as file:
            self.d_mt_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'j_metatitle_en.yaml') as file:
            self.j_mt_obj = remove_adult(yaml.safe_load(file))

        with open(self.path + 'tale_by_series_en.yaml') as file:
            self.tale_by_series_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'tale_en.yaml') as file:
            self.tale_obj = remove_adult(yaml.safe_load(file))
        with open(self.path + 'essay_en.yaml') as file:
            self.essay_obj = remove_adult(yaml.safe_load(file))
        self.other_obj = self.essay_obj

        with open(self.path + 'supplement_en.yaml') as file:
            self.supp_obj = remove_adult(yaml.safe_load(file))

    def merge_obj(self):
        self.scp_obj = ChainMap(self.scp_en_obj,
                                self.scp_001_obj,
                                self.ex_obj,
                                self.arc_obj,
                                self.d_obj,
        ...
        self.scp_mt_obj = ChainMap(self.scp_en_mt_obj,
                                   self.scp_001_mt_obj,
                                   self.ex_mt_obj,
                                   self.arc_mt_obj,
                                   self.d_mt_obj,
...
class MarkdownWriter():
    ...
    def format_object_link(self, link: str) -> str:
        ...
        result = f'    - [{title}]({common.en_url}{link}) '
        if rate != 'unknown':
    ...
    def format_each_scp(self, key: str) -> str:
        ...
        result += f'### [{scp_title.upper()}]({common.en_url + key})\n' + \
        ...
        for i in range(0, 6):
            ...
            for j in range(0, 10):
                ...
                for k in range(0, 100):
                    ...
                    scp_title = 'scp-' + scp_id
                    if scp_title in self.scp.scp_obj.keys():
                        result += self.format_each_scp(scp_title) + '\n'
            with open(self.path + f'scp_by_series/series{i + 1}.md', 'w', encoding='utf-8') as file:
                file.write(result)
    ...
    def save_to_markdown(self) -> None:
        ...
        with open(self.path + 'other_scps/joke.md', 'w', encoding='utf-8') as file:
            file.write(j_result)
...
class GraphDataWriter():
    ...
    def create_points(self) -> np.ndarray:
        keys = list(self.scp.scp_en_obj.keys())
        data_store = np.empty((len(keys), 8), dtype='object')
        for i in range(len(keys)):
            key = keys[i]
            num = int(key.replace('scp-', ''))
            ...
            data_store[i][7] = f'https://iwasaki501.github.io/ternbusty/scp_by_series/series' + \
